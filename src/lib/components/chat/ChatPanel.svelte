...
<section class:compact={variant === 'compact'} class="chat-panel" aria-label={$_('chat.title')}>
	<header>
		<div><span class="status-dot" aria-hidden="true"></span><h2 class="type-body-small">{$_('chat.title')}</h2></div>
	...
	<div bind:this={scroller} class="chat-log" aria-live="polite" aria-busy={pending}>
			...
			<div class="suggestions">
				...
				<div>
					{#each suggestions as suggestion (suggestion)}
						<button class="type-body-small" type="button" onclick={() => ask(suggestion)} disabled={pending}>{suggestion}</button>
					{/each}
				</div>
			</div>
		{/if}
		...
		{#if chat.errorKind}
			<div class="chat-error" role="alert">
				<p class="type-body-small">{chat.errorKind === 'rate-limit' ? $_('chat.rateLimited') : $_('chat.error')}</p>
				<button class="type-control" type="button" onclick={() => chat.retry()}>{$_('chat.retry')} →</button>
			</div>
		{/if}
	</div>
	...
	<form onsubmit={(event) => { event.preventDefault(); submit(); }}>
		...
		<textarea class="type-control" id={`ask-doto-input-${variant}`} bind:value={input} {onkeydown} rows={variant === 'compact' ? 1 : 2} maxlength="2000" placeholder={$_('chat.placeholder')}></textarea>
		<button type="submit" disabled={pending || !input.trim()} aria-label={$_('chat.send')}>{pending ? '…' : '↑'}</button>
	</form>

	{#if variant === 'compact' && hasQuestion}
		<a class="continue-link type-control" href={localizeHref('/ask', locale)}>{$_('askPreview.continue')} <span aria-hidden="true">↗</span></a>
	{/if}
</section>
...
<style>
	...
	.suggestions button { min-height: 3.5rem; padding: .75rem .85rem; border: 1px solid var(--color-line); background: rgba(255,255,255,.025); color: var(--color-muted); text-align: left; transition: border-color 180ms, color 180ms; }
	...
	.chat-error p { margin: 0; color: var(--color-ink); }
	.chat-error button { min-height: 2.75rem; margin-top: .25rem; border: 0; background: transparent; color: var(--color-secondary); }
	...
	textarea::placeholder { color: var(--color-subtle); }
	...
	.continue-link { display: flex; justify-content: space-between; align-items: center; min-height: 3.5rem; padding: 0 1.25rem; border-top: 1px solid var(--color-line); color: var(--color-secondary); font-weight: 600; }
