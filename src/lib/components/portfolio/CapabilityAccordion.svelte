	...
	{#each groups as group, index (group.category)}
		<div class="capability-item">
			...
			>
				...
				<strong>{pick(group.title, locale)}</strong>
				<span>{group.items.slice(0, 3).map((item) => item.name).join(' · ')}</span>
			...
			{#if active === index}
				<div class="capability-detail" id={panelId(group.category)}>
					<p>{pick(group.description, locale)}</p>
					<div>
						<p class="meta-label">{$_('capabilities.evidence')}</p>
						<ul>
							{#each evidence(group) as project (project.slug)}
								<li><a href={localizeHref(`/projects/${project.slug}`, locale)}>{pick(project.title, locale)} <span aria-hidden="true">↗</span></a></li>
							{/each}
						</ul>
					</div>
					<ul class="tool-list" aria-label="Tools">
						{#each group.items as item (item.name)}<li>{item.name}</li>{/each}
					</ul>
				</div>
			{/if}
		</div>
...
<style>
	...
	.number, .capability-row span, .capability-row small { color: var(--color-muted); font-family: var(--font-mono); font-size: .69rem; }
	.capability-row strong { font-size: clamp(1.8rem, 3vw, 3.4rem); letter-spacing: -.05em; font-weight: 600; }
	...
	.capability-detail > p { max-width: 44rem; margin: 0; color: var(--color-muted); line-height: 1.75; }
	.capability-detail ul { margin: .9rem 0 0; padding: 0; list-style: none; }
	.capability-detail a { display: inline-block; padding: .25rem 0; color: var(--color-ink); font-size: .86rem; text-decoration: underline; text-decoration-color: var(--color-line-strong); text-underline-offset: .3rem; }
	.tool-list { grid-column: 1 / -1; display: flex; flex-wrap: wrap; gap: .5rem; }
	.tool-list li { padding: .45rem .7rem; border: 1px solid var(--color-line); border-radius: 999px; font-family: var(--font-mono); font-size: .68rem; }
