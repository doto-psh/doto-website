...
<div class="system-map">
	<div class="system-map-head type-meta">
	...
	{#if flow}
		<ol aria-label={pick(flow.title, locale)}>
			{#each steps as step, index (step.label)}
				<li>
					<span class="node-index type-meta-decorative">{String(index + 1).padStart(2, '0')}</span>
					<div>
						<strong class="type-body-small">{step.label}</strong>
						{#if step.detail}<small class="type-meta">{step.detail}</small>{/if}
					</div>
				</li>
			{/each}
		</ol>
	{/if}
	<div class="system-map-foot type-meta">
...
<style>
	...
	.system-map-head, .system-map-foot { display: flex; justify-content: space-between; gap: 1rem; padding: 1.1rem 1.4rem; letter-spacing: .06em; text-transform: uppercase; }
	...
	.node-index { display: grid; position: relative; z-index: 1; width: 3rem; height: 3rem; margin: 0 auto 1.1rem; place-items: center; border: 1px solid rgba(86,137,255,.65); border-radius: 50%; background: #111823; color: var(--color-secondary); box-shadow: 0 0 1.8rem rgba(86,137,255,.12); }
	strong { display: block; font-weight: 600; }
	small { display: block; margin-top: .35rem; color: var(--color-subtle); }
