...
<header class:scrolled class="site-header">
	...
	<div class="header-grid">
		<a href={localizeHref('/', locale)} data-no-visited class="brand type-control" aria-label={`${site.brand} home`}>
		...
		<nav class="desktop-nav" aria-label={$_('nav.primary')}>
			{#each navigation as item (item.key)}
				<a class="type-control" href={localizeHref(item.path, locale)} data-no-visited aria-current={isActive(item.path) ? 'page' : undefined}>{$_(`nav.${item.key}`)}</a>
			{/each}
		</nav>

		<div class="header-actions">
			<a class="ask-link type-control" href={localizeHref('/ask', locale)} data-no-visited>{$_('nav.ask')} <span aria-hidden="true">↗</span></a>
...
{#if menuOpen}
	<div bind:this={menuPanel} class="mobile-menu" role="dialog" aria-modal="true" aria-label={$_('nav.primary')} tabindex="-1" onkeydown={trapFocus}>
		<div class="mobile-menu-head">
			<a href={localizeHref('/', locale)} class="brand type-control" data-no-visited><span class="brand-mark">d</span><span>{site.brand}</span><i aria-hidden="true"></i></a>
			<button class="type-control" type="button" onclick={() => closeMenu()}>{$_('nav.close')}</button>
		</div>
		<nav id="mobile-navigation">
			{#each navigation as item, index (item.key)}
				<a class="type-section-title" href={localizeHref(item.path, locale)} data-no-visited><span class="type-meta-decorative">{String(index + 1).padStart(2, '0')}</span>{$_(`nav.${item.key}`)}<i class="type-meta-decorative" aria-hidden="true">↗</i></a>
			{/each}
			<a class="type-section-title" href={localizeHref('/ask', locale)} data-no-visited><span class="type-meta-decorative">06</span>{$_('nav.ask')}<i class="type-meta-decorative" aria-hidden="true">↗</i></a>
		</nav>
		<div class="mobile-menu-foot"><p class="type-meta">AI Engineer · Seoul, Korea</p><LanguageSwitcher {locale} /></div>
	</div>
{/if}
...
<style>
	...
	.brand { display: inline-flex; width: fit-content; align-items: center; gap: .55rem; color: #f3f6fa; font-weight: 650; }
	...
	.desktop-nav a { position: relative; padding: 1rem 0; color: #b8c2d0; font-weight: 500; transition: color 180ms; }
	...
	.ask-link { min-height: 2.75rem; display: inline-flex; align-items: center; gap: .65rem; padding: 0 1rem; border: 1px solid var(--color-line); border-radius: 999px; color: #e1e7ef; transition: border-color 180ms, background 180ms; }
	...
	.mobile-menu-head button { min-width: 2.75rem; min-height: 2.75rem; border: 0; background: transparent; color: #fff; }
	.mobile-menu nav { border-top: 1px solid var(--color-line); }
	.mobile-menu nav a { display: grid; grid-template-columns: 2.5rem 1fr auto; align-items: baseline; padding: 1.05rem 0; border-bottom: 1px solid var(--color-line); color: #fff; }
	.mobile-menu nav span, .mobile-menu nav i { font-style: normal; }
	...
	.mobile-menu-foot p { max-width: 12rem; margin: 0; }
