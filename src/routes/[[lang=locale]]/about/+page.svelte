...
<header class="about-hero">
	<div class="container-page about-grid">
		<div class="portrait-wrap">
			...
			<div><span class="type-meta">{pick(profile.location, data.locale)}</span><span class="type-meta">{pick(profile.role, data.locale)}</span></div>
		</div>
		<div class="intro">
			<p class="meta-label">{$_('aboutPage.eyebrow')} · PROFILE</p>
			<h1 class="type-page-title">{$_('aboutPage.title')}</h1>
			<p class="lead type-lead">{$_('aboutPage.intro')}</p>
			<div class="bio prose-reading">{#each bioParagraphs as paragraph (paragraph)}<p class="type-prose">{paragraph}</p>{/each}</div>
		</div>
	</div>
</header>
...
<section id="experience" class="experience-section" aria-labelledby="experience-title">
	<div class="container-page section-grid">
		<div><p class="meta-label">01 / Career</p><h2 class="type-section-title" id="experience-title">{$_('aboutPage.timeline')}</h2></div>
...
<section class="theme-light skills-section" aria-labelledby="skills-title">
	<div class="container-page section-grid">
		<div><p class="meta-label">02 / Stack</p><h2 class="type-section-title" id="skills-title">{$_('aboutPage.skills')}</h2></div>
		<div class="skill-list">
			{#each data.portfolio.skills as group, index (group.category)}
				<article>
					<span class="type-meta-decorative">{String(index + 1).padStart(2, '0')}</span>
					<div><h3 class="type-lead">{pick(group.title, data.locale)}</h3><p class="type-body-small">{pick(group.description, data.locale)}</p></div>
					<ul>{#each group.items as item (item.name)}<li class="type-meta">{item.name}</li>{/each}</ul>
				</article>
			{/each}
		</div>
	</div>
...
<section class="support-section" aria-label={$_('aboutPage.timeline')}>
	<div class="container-page section-grid">
		<div><p class="meta-label">03 / More</p><h2 class="type-section-title">Background</h2></div>
		<div class="support-list">
			{#each supporting as item (item.id)}
				<article>
					<p class="type-meta">{$_(`portfolio.timelineCategories.${item.category}`)}</p>
					<div><h3 class="type-lead">{pick(item.title, data.locale)}</h3>{#if item.organization}<span class="type-meta">{pick(item.organization, data.locale)}</span>{/if}</div>
					<time class="type-meta">{item.period ? pick(item.period, data.locale) : ''}</time>
				</article>
			{/each}
		</div>
	</div>
...
<section class="theme-light contact-section" aria-labelledby="contact-title">
	<div class="container-page contact-grid">
		<div><p class="meta-label">{$_('aboutPage.contact')}</p><h2 class="type-section-title" id="contact-title">{$_('contact.title')}</h2></div>
		<div class="contact-links">
			<a class="type-control" href={`mailto:${profile.email}`}>{profile.email}<span>↗</span></a>
			<a class="type-control" href={profile.resumeUrl}>{$_('aboutPage.resume')}<span>↗</span></a>
			<a class="type-control" href={profile.github} target="_blank" rel="noreferrer">GitHub<span>↗</span></a>
		</div>
	</div>
</section>
...
<style>
	...
	.portrait-wrap > div { display: flex; justify-content: space-between; gap: 1rem; margin-top: 1rem; text-transform: uppercase; }
	.meta-label { color: var(--color-secondary); }
	.intro h1 { max-width: 11ch; margin: 1.5rem 0 0; }
	.lead { max-width: 40rem; margin: 2rem 0 0; color: #c9d1dc; }
	.bio { max-width: 43rem; margin-top: 3.5rem; padding-top: 2rem; border-top: 1px solid var(--color-line); }
	.bio p { margin: 0; color: var(--color-muted); }
	...
	.section-grid h2 { margin: 1rem 0 0; }
	...
	.skill-list h3 { margin: 0; font-weight: 650; }
	.skill-list p { margin: .7rem 0 0; color: var(--color-muted); }
	.skill-list ul { display: flex; flex-wrap: wrap; align-content: start; gap: .45rem; margin: 0; padding: 0; list-style: none; }
	.skill-list li { padding: .35rem .55rem; border: 1px solid var(--color-line); border-radius: 999px; color: var(--color-subtle); }
	...
	.support-list p, .support-list time, .support-list span { margin: 0; }
	.support-list h3 { margin: 0 0 .45rem; font-weight: 600; }
	...
	.contact-grid h2 { max-width: 17ch; margin: 1rem 0 0; }
	.contact-links { border-top: 1px solid var(--color-line); }
	.contact-links a { display: flex; min-height: 3.25rem; justify-content: space-between; align-items: center; gap: 1rem; padding: .75rem 0; border-bottom: 1px solid var(--color-line); color: var(--color-ink); }
