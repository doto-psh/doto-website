...
<section class="statement section-shell">
	<div class="container-page statement-grid reveal" use:reveal>
		<div>
			...
			<p class="statement-note type-meta">{$_('statement.note')}</p>
		</div>
		<div class="statement-copy">
			{#each pick(data.home.statement, data.locale) as line, index (line)}
				<p class="type-page-title" class:accent={index === 1} class:muted={index === 2}>{line}</p>
			{/each}
		</div>
	</div>
</section>
...
<section id="selected-work" class="section-shell work-section">
	<div class="container-page reveal" use:reveal>
		<div class="section-heading">
			<div><p class="meta-label text-[var(--color-secondary)]">{$_('featured.eyebrow')}</p><h2 class="type-section-title">{$_('featured.title')}</h2></div>
			<p class="type-body">{$_('featured.subtitle')}</p>
		</div>
...
<section class="section-shell architecture-section">
	<div class="container-page architecture-grid reveal" use:reveal>
		<div class="architecture-copy">
			...
			<h2 class="type-section-title">{$_('architecture.title')}</h2>
			<p class="type-body">{$_('architecture.subtitle')}</p>
...
<section id="capabilities" class="theme-light section-shell">
	<div class="container-page reveal" use:reveal>
		<div class="section-heading section-heading-light">
			<div><p class="meta-label text-[var(--color-accent)]">{$_('capabilities.eyebrow')}</p><h2 class="type-section-title">{$_('capabilities.title')}</h2></div>
			<p class="type-body">{$_('capabilities.subtitle')}</p>
		</div>
...
<section class="section-shell experience-section">
	<div class="container-page experience-grid reveal" use:reveal>
		<div class="experience-copy">
			...
			<h2 class="type-section-title">{$_('experiencePreview.title')}</h2>
			<p class="type-body">{$_('experiencePreview.subtitle')}</p>
...
<section class="section-shell notes-section">
	<div class="container-page reveal" use:reveal>
		<div class="section-heading">
			<div><p class="meta-label text-[var(--color-secondary)]">{$_('notesPreview.eyebrow')}</p><h2 class="type-section-title">{$_('notesPreview.title')}</h2></div>
		...
		<ol class="note-list">
			{#each data.posts as post, index (post.slug)}
				<li><a href={localizeHref(`/notes/${post.slug}`, data.locale)}><span class="note-index type-meta-decorative">{String(index + 1).padStart(2, '0')}</span><time class="type-meta" datetime={post.date}>{formatDate(post.date)}</time><div><h3 class="type-item-title">{pick(post.title, data.locale)}</h3><p class="type-body-small">{pick(post.excerpt, data.locale)}</p></div><i aria-hidden="true">↗</i></a></li>
			{/each}
		</ol>
	</div>
</section>
...
<section class="theme-light section-shell about-preview">
	<div class="container-page about-grid reveal" use:reveal>
		<div><p class="meta-label text-[var(--color-accent)]">{$_('aboutPreview.eyebrow')}</p><h2 class="type-section-title">{$_('aboutPreview.title')}</h2></div>
		<div><p class="type-body">{$_('aboutPreview.body')}</p><p class="about-detail type-body">{pick(profile.bio, data.locale).split('\n\n')[0]}</p><a class="text-link" href={localizeHref('/about', data.locale)}>{$_('aboutPreview.cta')} <span aria-hidden="true">→</span></a></div>
	</div>
</section>

<section class="section-shell ask-section">
	<div class="container-page ask-grid reveal" use:reveal>
		<div class="ask-copy"><p class="meta-label text-[var(--color-secondary)]">{$_('askPreview.eyebrow')}</p><h2 class="type-section-title">{$_('askPreview.title')}</h2><p class="type-body">{$_('askPreview.body')}</p></div>
...
<style>
	...
	.section-heading h2, .architecture-copy h2, .experience-copy h2, .about-grid h2, .ask-copy h2 { max-width: 13ch; margin: 1.1rem 0 0; }
	.section-heading > p, .section-heading > a, .architecture-copy > p, .experience-copy > p, .about-grid p, .ask-copy > p { margin: 0; color: var(--color-muted); }
	...
	.statement-note { max-width: 15rem; margin: 1.25rem 0 0; color: var(--color-subtle); }
	...
	.note-list time { color: var(--color-subtle); }
	.note-list h3 { margin: 0; }
	.note-list p { max-width: 45rem; margin: .65rem 0 0; color: var(--color-muted); }
